...
\;[x]_{D+1; P} &=& [x_0, x_{11}, \dots, x_{1P}, x_{21}, \dots, x_{DP}] \quad \quad \mbox{vectorized} \;.
\end{eqnarray*}
...
{\scriptsize
\begin{verbatim}
inline int li(int d, int p, int P){
    /* computes linear index li of vectorized Taylor polynomial
    Do not use this for optimized algorithms.
    */
    return (d>0)*(1 + p + P*(d-1));
}

int amul_nonopt(int D, int P, double *x, double *y, double *z ){
    /* 
    assignment multiplication non optimized
    computes  z += mul(x,y) in Taylor arithmetic, not optimized
    */
    int k,d,p;
    
    /* d = 0:  compute base point */
    z[0] += x[0] * y[0];
    ...
    for(d = 0; d != D; ++d){
        for(k = 0; k != d; ++k){
            for(p = 0; p != P; ++p){
                z[li(d,p,P)] += x[li(k,p,P)] * y[li(d-k,p,P)];
            }
        }
    }
