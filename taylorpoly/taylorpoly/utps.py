...
def utps_ndarray_get_coeff(x):
    """

    y, S = utps_ndarray_get_coeff(x)

    Parameters
    ----------

    :x: array-like with UTPS entries
        x.shape = (N,)
    ...
    :y: numpy.ndarray
         y.shape = (N,), dtype=float
         [x[0].data[0], ..., x[N].data[0]]

    :S:  numpy.ndarray
         S.shape = (N,P,D), dtype=float
